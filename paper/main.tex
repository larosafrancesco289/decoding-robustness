...
\title{Temperature Robustness Is a Model Property,\\Not a Sampler Choice}
% Title alternatives:
...
accuracy collapse of high-temperature sampling. Sampler evaluations compare one rule
against another and take the collapse for granted. We ask whether it occurs in
every model at temperatures used in deployment ($T \le 1.3$). In a single controlled
pipeline, we run seven instruction-tuned models from five families across 8 sampler
arms, 4 quantization levels, and 2 tasks. In that range, sampler choice barely matters
in every family except Llama. Under plain temperature sampling at $T{=}1.3$,
...
decoding. A temperature ladder up to $T{=}2.0$ shows that every model collapses at some
temperature, and truncation samplers begin to matter only past it. Forcing one
off-distribution token into the context multiplies a fragile model's chance of drawing
another, so errors compound. Practitioners tuning a current model can leave the sampler
alone.
\end{abstract}
...
\textbf{whether the collapse is a property of decoding, or of the model}.

At each step the decoder draws the next token at random from the model's predicted
distribution, and a temperature parameter scales the randomness of the draw. Raising it
...
before the draw, and the rules differ in where they cut. Top-$k$, nucleus (top-$p$),
min-$p$, and top-$n\sigma$ are the four we test, and Section~\ref{sec:related}
describes each.
...
0.6 and 1.0 (Section~\ref{sec:setup}). \citet{schaeffer2025minp} reanalyzed min-$p$ and
found the samplers hard to separate once each one receives an equal tuning budget, but
the reanalysis still asks which sampler wins, and leaves the premise itself untouched.
