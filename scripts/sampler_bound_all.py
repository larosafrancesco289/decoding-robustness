...
def main():
    ...
    def summary(keys, label):
        ...
        above = sum(o["lo"] > 0 for o in rows)
        below = sum(o["hi"] < 0 for o in rows)
        ...
        maxes = mats[:, idx].mean(axis=2).max(axis=0)  # per bootstrap draw, max over comparisons
        sim = float(np.percentile(maxes, 95))
        within3 = sum(abs(o["mean"]) <= 3 for o in rows)
        print(f"{label}: n={len(rows)}  max gain {mx['mean']:+.1f} ({mx['model']} {mx['task']} T{mx['T']} {mx['arm']}); "
              f"max upper {mu['hi']:+.1f} ({mu['model']} {mu['task']} T{mu['T']} {mu['arm']}); simultaneous 95% bound on the largest gain {sim:+.1f}; "
              f"CIs above zero {above}, below {below}; |mean|<=3: {within3}")
        return dict(label=label, n=len(rows), max_gain=mx["mean"], max_upper=mu["hi"], simultaneous=sim, above=above, below=below, within3=within3)
    ...
    keys = [k for k in diffs if k[3] in (0.7, 1.0)]
